...
from memory_store import get_memory_by_user, save_memory
...
def output_success(state: AgentState) -> AgentState:
    result_msg = "分析成功 ✅，情绪正面率达标！"
    send_wecom_markdown("🎉 情绪分析成功", result_msg)
    return {**state, "final_result": result_msg}
...
def suggest_improve(state: AgentState) -> AgentState:
    ...
    send_wecom_markdown("📌 情绪改进建议", improvement)
...
def run_sentiment_loop(comments: list[str], user_id: str) -> dict:
    ...
    save_memory(user_id, result)  # 保存最终状态到 Redis
    return result
