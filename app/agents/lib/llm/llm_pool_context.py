...
async def chain_example_s():
    ...
    # 执行链
    results = await executor.execute_node(summarize_node, input_data, ResponseModel)
...
async def parallel_chain_example():
    ...
    translate_en_fr_node = ChainNode(
        "gpt-3.5-turbo",
        "Translate this text to French: {input}",
        name="English-French"
    )
    ...
    parallel_results = await executor.execute_parallel(
        [summarize_node, translate_en_zh_node, translate_en_fr_node, sentiment_node],
        input_data,
        ResponseModel
    )
...
if __name__ == "__main__":
    import asyncio
    asyncio.run(parallel_chain_example())
