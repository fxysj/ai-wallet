...
- 返回完整的表单信息，并生成自然、亲切、有温度的回复，引导用户补充信息。
...
5. 生成语气自然、有亲和力的回复，引导用户补全缺失信息，让用户感觉到贴心帮助。
...
json
{{"data": {{
    "description": "用自然语言向用户解释当前收集情况。例如：'我们已经记录了大部分信息，还差一点就完成啦！请帮我补充一下xxx~'（务必根据语言类型翻译）如果字段完整则返回一切都已准备好，敬请享受转账的快乐！（务必根据语言类型翻译）",
    "state": "{{
